...
class AppStore extends Component {
  ...
  getCategoryApps = () => {
    ...
    const filterActiveApps = appsList.filter((each) => {
      return each.category === activeTab;
    });
    return filterActiveApps;
  };
  onChangeTab = (id) => {
    this.setState({
      activeTab: id,
  ...
  render() {
    ...
    const categoryApps = this.getCategoryApps();
    const filterApps = categoryApps.filter((each) => {
      return each.appName
        .toLocaleLowerCase()
    ...
    return (
      <div className="container">
          ...
          {tabsList.map((each) => {
            return (
              <AppButtons
                each={each}
                key={each.tabId}
                activeTabId={each.tabId === activeTab}
        ...
        <ul className="app-list-card">
          {filterApps.length > 0 ? (
            filterApps.map((each) => {
              return <AppItems each={each} key={each.appId} />;
            })
          ) : (
            <b>
              No Apps Found <span>{searchValue}</span>
            </b>
          )}
        </ul>
      </div>
